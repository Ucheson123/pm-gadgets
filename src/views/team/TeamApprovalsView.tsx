  ...
  return (
    <div className="space-y-6">
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl overflow-hidden shadow-sm transition-colors duration-200">
        <div className="p-6 border-b border-slate-200 dark:border-slate-800 flex justify-between items-center transition-colors duration-200">
          <h2 className="text-lg font-bold text-slate-900 dark:text-white transition-colors">Pending Team Approvals</h2>
          <span className="px-3 py-1 bg-orange-100 text-orange-700 dark:bg-orange-900/30 dark:text-orange-400 rounded-full text-sm font-semibold transition-colors">
        ...
        {pendingUsers.length === 0 ? (
          <div className="p-12 text-center text-slate-500 dark:text-slate-400 transition-colors">
            <CheckCircle2 className="w-12 h-12 mx-auto mb-3 text-slate-300 dark:text-slate-700 transition-colors" />
          ...
          <div className="divide-y divide-slate-200 dark:divide-slate-800 transition-colors duration-200">
            {pendingUsers.map((user) => {
              ...
              return (
                <div
                  key={user.id}
                  className="p-6 flex items-center justify-between hover:bg-red-50/50 dark:hover:bg-red-500/5 transition-colors duration-200"
                >
                  <div className="flex items-center gap-4">
                    <div className="w-12 h-12 bg-slate-100 dark:bg-slate-800 rounded-full flex items-center justify-center text-slate-700 dark:text-slate-300 font-bold uppercase transition-colors duration-200">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-slate-500 dark:text-slate-400 capitalize flex items-center gap-1.5 transition-colors">
                        Requested role: {user.role}
                        {user.role === 'manager' && (
                          <span className="inline-flex items-center gap-1 px-2 py-0.5 bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400 rounded-full text-xs font-semibold transition-colors">
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      ...
                      className="flex items-center gap-2 px-4 py-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 hover:bg-red-50 hover:text-red-600 hover:border-red-100 dark:hover:bg-red-500/10 dark:hover:text-red-400 dark:hover:border-red-500/20 disabled:opacity-50 rounded-lg text-sm font-medium transition-all duration-200"
                    >
                      <X size={16} /> Reject
                    </button>
                    <button
        ...
        actions={
          <>
            ...
            <button
              ...
              className="px-4 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl text-sm font-semibold transition-colors shadow-sm"
            >
              Yes, Approve Manager
            </button>
          </>
      ...
      >
        <p className="text-slate-600 dark:text-slate-300 transition-colors">
          <strong className="text-slate-900 dark:text-white">{confirmManager?.full_name || 'This user'}</strong> requested the{' '}
        ...
        <p className="mt-3 text-sm text-slate-400 dark:text-slate-500 transition-colors">
