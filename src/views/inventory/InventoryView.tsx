...
const StockBadge = ({ quantity }: { quantity: number }) => {
  if (quantity === 0) {
    return (
      <span className="px-2.5 py-1 bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400 rounded-full text-xs font-semibold">
        Out of Stock
      </span>
    );
  }
  if (quantity < LOW_STOCK_THRESHOLD) {
    return (
      <span className="px-2.5 py-1 bg-orange-100 text-orange-700 dark:bg-orange-900/30 dark:text-orange-400 rounded-full text-xs font-semibold">
        Low Stock
      </span>
    );
  }
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl overflow-hidden shadow-sm">
        <div className="p-5 border-b border-slate-200 dark:border-slate-800 flex justify-between items-center">
          <h2 className="font-bold text-slate-900 dark:text-white">Branch Stock</h2>
          {stock && (
        ...
        {!isLoading && !isError && filteredStock.length > 0 && (
          <div className="divide-y divide-slate-200 dark:divide-slate-800">
            {filteredStock.map((row) => (
              <div
                key={row.id}
                className="p-5 flex flex-col sm:flex-row sm:items-center gap-3 sm:justify-between hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors"
              >
                  ...
                  {isManager && (
                    <div className="flex items-center gap-2">
                      <button
                        ...
                        className="flex items-center gap-2 px-3 py-2 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 rounded-lg text-sm font-medium transition-colors"
                      >
                        <Pencil size={15} /> Edit
                      </button>
                      <button
