        ...
        <>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <KpiCard
              ...
              value={formatNaira(s.wallet_balance)}
              icon={WalletIcon}
            />
            <KpiCard
              label="Total Revenue"
              value={formatNaira(s.total_revenue)}
              icon={TrendingUp}
              accent="text-emerald-600"
            />
            <KpiCard
              label="Total Expenses (COGS)"
              value={formatNaira(s.total_expenses)}
              icon={TrendingDown}
              accent="text-red-600"
            />
            <KpiCard
              label="Net Profit"
              value={formatNaira(s.total_revenue - s.total_expenses)}
              icon={Banknote}
              accent={
                s.total_revenue - s.total_expenses >= 0
                  ? "text-emerald-600"
                  : "text-red-600"
              }
            />
            ...
            <KpiCard
              ...
              value={formatNaira(s.inventory_value)}
              icon={Package}
            />
            <KpiCard
                ...
                <div className="divide-y divide-slate-100 dark:divide-slate-800">
                  {s.recent_transactions.map((tx) => (
                    ...
                    >
                      ...
                      <span
                        className={`font-semibold shrink-0 ${tx.type === "credit" ? "text-emerald-600" : "text-red-600"}`}
                      >
                        {tx.type === "credit" ? "+" : "−"}
        ...
        <>
          <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
            <KpiCard
              ...
              value={formatNaira(combined.revenue)}
              icon={TrendingUp}
              accent="text-emerald-600"
            />
            <KpiCard
              label="Combined Expenses"
              value={formatNaira(combined.expenses)}
              icon={TrendingDown}
              accent="text-red-600"
            />
            <KpiCard
              label="Combined Wallets"
              value={formatNaira(combined.wallets)}
              icon={WalletIcon}
            />
            <KpiCard
              label="Combined Sales"
              value={String(combined.sales)}
              icon={ReceiptText}
            />
            <KpiCard
              label="Combined Inventory"
              value={formatNaira(combined.inventory)}
              icon={Package}
            />
          </div>
          ...
          <ChartCard title="Branch Comparison — Revenue vs Expenses">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={company.data}>
                ...
                <YAxis tickFormatter={compactNaira} fontSize={11} width={70} />
                <Tooltip
                  {...chartTooltip}
                  formatter={(v) => formatNaira(Number(v))}
                />
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl overflow-hidden">
            ...
            <div className="divide-y divide-slate-100 dark:divide-slate-800">
              {company.data.map((b) => (
                <div
                  key={b.branch_id}
                  className="p-4 grid grid-cols-2 md:grid-cols-5 gap-2 text-sm"
                >
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <KpiCard
          ...
          icon={AlertTriangle}
          accent={lowStock > 0 ? "text-orange-600" : undefined}
        />
        <KpiCard
          label="Out of Stock"
          value={String(outOfStock)}
          icon={TrendingDown}
          accent={outOfStock > 0 ? "text-red-600" : undefined}
        />
      </div>
    </div>
  );
